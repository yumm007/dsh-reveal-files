...
async function showInTerminal(targets, timeoutMs) {
    ...
    if (platform === 'darwin') {
        // osascript + Terminal.app: activate, then plain `do script` —
        // user-verified behavior on this host is a NEW WINDOW each time
        // (Terminal allocates the front window's selected tab but Terminal
        // itself opens a fresh window for the command). This is the chosen
        // default: no Accessibility permission, no tab pollution, and every
        // trigger gets its own window with the file's directory as cwd.
