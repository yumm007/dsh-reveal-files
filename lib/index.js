...
/**
 * Display one set of targets in a native terminal window, echoing each path.
 * @param targets - absolute filesystem paths to print.
 * @param timeoutMs - per-command timeout.
 * @returns the command list actually spawned (report only).
 */
/**
 * Display one set of targets in a native terminal window: the window cd's
 * into the first target's directory and prints each path.
 * @param targets - absolute filesystem paths to print.
 * @param timeoutMs - per-command timeout.
 * @returns the command list actually spawned (report only).
 */
async function showInTerminal(targets, timeoutMs) {
    ...
    // writes into one directory); every path is still printed after cd.
    const at = Math.max(targets[0].lastIndexOf('/'), targets[0].lastIndexOf('\\'));
    const dir = at > 0 ? targets[0].slice(0, at) : '.';
    const echoed = targets.map((t) => shQuote(t)).join(' ');
    const script = `cd ${shQuote(dir)} && echo ${echoed}`;
    if (platform === 'darwin') {
        // osascript + Terminal.app: a new window runs `cd <dir> && echo ...`.
        commands.push(['osascript', ['-e', `tell application "Terminal" to do script "${script.replace(/"/g, '\\"')}"`], script]);
    } else if (platform === 'win32') {
        const winScript = `cd /d "${String(dir).replace(/"/g, '"^"')}" && echo ${targets.map((t) => `"${String(t).replace(/"/g, '"^"')}"`).join(' ')}`;
...
export function apply(ctx, config = {}) {
    ...
    async function handle(req, res, operation) {
        ...
        try {
            ...
            res.writeHead(error && error.status === 413 ? 413 : 400, { 'content-type': 'application/json' });
        ...
        if (targets.length === 0) {
            ...
            res.end(JSON.stringify({ ok: false, error: 'no paths' }));
            return;
        }
        let outcome;
        try {
