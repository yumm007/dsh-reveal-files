...
async function showInTerminal(targets, timeoutMs) {
    ...
    if (platform === 'darwin') {
        // osascript + Terminal.app. Measured behavior on this host:
        //   - plain `do script` opens a NEW WINDOW even when Terminal runs;
        //   - `do script ... in front window` runs in the CURRENT tab,
        //     polluting it (tab is a read-only element — no `make new tab`).
        // To always get a genuinely NEW tab in the running Terminal: send
        // Cmd+T via System Events, then `do script ... in front window` (the
        // new tab becomes the front window's selected tab). Requires the
        // Accessibility permission for System Events. When Terminal is not
        // running at all (count of windows = 0), plain `do script` creates
        // the first window without needing that permission.
        const run = script.replace(/"/g, '\\"');
        const osaScript = [
