...
async function showInTerminal(targets, timeoutMs) {
    ...
    if (platform === 'darwin') {
        // osascript + Terminal.app: activate, then reuse the front window —
        // `do script ... in window 1` opens a NEW TAB in the existing front
        // window when one exists; only when Terminal has no window at all
        // do we fall back to `do script` (which creates a new window).
        // So: Terminal already open → new tab in it; not open → new window.
        const run = script.replace(/"/g, '\\"');
        const osaScript = `tell application "Terminal"\n\tactivate\n\tif (count of windows) = 0 then\n\t\tdo script "${run}"\n\telse\n\t\tdo script "${run}" in window 1\n\tend if\nend tell`;
