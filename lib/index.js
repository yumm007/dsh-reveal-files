...
async function showInTerminal(targets, timeoutMs) {
    ...
    const commands = [];
    if (platform === 'darwin') {
        // osascript + Terminal.app: a new window runs `echo <path...>`.
        const script = `echo ${targets.map((t) => shQuote(t)).join(' ')}`;
        commands.push(['osascript', ['-e', `tell application "Terminal" to do script "${script.replace(/"/g, '\\"')}"`], `echo ${script}`]);
    } else if (platform === 'win32') {
        const echoed = targets.map((t) => `"${String(t).replace(/"/g, '"^"')}"`).join(' ');
        commands.push(['cmd', ['/c', 'start', 'cmd', '/k', `echo ${echoed}`], `echo ${echoed}`]);
    } else if (platform === 'linux') {
        // Try known emulators in order; the first that exists wins. The
        // command keeps the window open until the user presses Enter.
        const script = `echo ${targets.map((t) => shQuote(t)).join(' ')}; read -p 'Press Enter to close'`;
        for (const bin of LINUX_TERMINALS) {
            commands.push([bin, ['-e', script], `${bin} -e ${script}`]);
        }
    } else {
