...
async function showInTerminal(targets, timeoutMs) {
    ...
    if (platform === 'darwin') {
        // osascript + Terminal.app. Measured behavior on this host: plain
        // `do script` (no `in`) opens a NEW WINDOW even when Terminal is
        // running. To reuse the running Terminal and only add a tab, target
        // the front window explicitly — `do script ... in front window` is a
        // legal `in` target (window) and means "the current front window".
        // Fall back to a plain `do script` only when no window exists yet.
