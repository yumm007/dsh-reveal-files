...
async function showInTerminal(targets, timeoutMs) {
    ...
    if (platform === 'darwin') {
        // osascript + Terminal.app. Plain `do script` (no `in window` clause)
        // is the documented Terminal.app behavior: when a window already
        // exists the command runs in a NEW TAB of the front window, and only
        // when no window exists does it create one. Activate first so the
        // window comes to the front. (An explicit `in window N` clause is
        // interpreted inconsistently across macOS versions and can open a
        // new window instead of a tab — avoid it.)
